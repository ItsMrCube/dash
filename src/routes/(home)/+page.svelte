<script lang="ts">
  ...
  const metrics: {
    ...
    {
      name: "Temperature",
      content: Intl.NumberFormat(undefined, {
        style: "unit",
        unit: "celsius",
      }).format(data.cpuTemperature.main),
...
<div class="grid gap-8">
  <div class="card flex items-center justify-center">
    <div>
      <p>
        The current temperature is {data.meteo.current.temp.toFixed(0)}°C, it
        feels like {data.meteo.current.feels_like.toFixed(0)}°C with {data.meteo
